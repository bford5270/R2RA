    ...
    <div className="flex items-center gap-2 py-1.5 border-b border-neutral-100 last:border-0">
      {isImage && blobUrl ? (
        <img
          src={blobUrl}
          alt={ev.filename}
          className="w-10 h-10 rounded object-cover shrink-0 border border-neutral-400 cursor-pointer"
          onClick={handleDownload}
          title="Click to download"
        />
      ) : (
        <div className="w-10 h-10 rounded bg-neutral-100 border border-neutral-400 shrink-0 flex items-center justify-center text-neutral-400">
...
export function EvidencePanel({ assessmentId, itemId }: Props) {
  ...
  return (
    <div className="mt-2 rounded border border-neutral-400 bg-neutral-50 px-3 py-2">
